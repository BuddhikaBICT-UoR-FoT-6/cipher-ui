...
const BruteForceSimulator = ({ isVisible, onClose, customCipherData }) => {
  ...
  const customMapping = customCipherData?.mapping;
  const customName = customCipherData?.name;
  ...
  useEffect(() => {
    if (isVisible) {
      ...
      if (customMapping && Object.keys(customMapping).length > 0) {
        const mappingEntries = Object.keys(customMapping).length;
        const mappingValues = Object.values(customMapping);
        ...
        const nonIdentityMappings = Object.entries(customMapping)
        ...
        cipherName = customName || 'Custom Cipher';
      } else {
        keySpace = 1;
  ...
  }, [isVisible, customMapping, customName]);
